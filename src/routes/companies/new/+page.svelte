...
<article class="mx-auto flex max-w-xl flex-col gap-4">
  <header>
    <h1 class="text-2xl font-semibold tracking-tight">New company</h1>
    <p class="text-sm text-[var(--color-muted)]">
      Or paste a website link in the topbar — it'll be classified and enriched automatically.
    </p>
  </header>
  ...
  >
    ...
    <label class="flex flex-col gap-1 text-sm">
      <span class="text-[var(--color-muted)]">Industry</span>
      <input name="industry" class={inputClass} />
    </label>
    <label class="flex flex-col gap-1 text-sm">
    ...
    <div class="flex items-center gap-2">
      ...
      >{submitting ? 'Saving…' : 'Save company'}</button>
      <a href="/companies" class="rounded-[var(--radius-sm)] border border-[var(--color-border)] px-4 py-2 text-sm">Cancel</a>
    </div>
  </form>
</article>
