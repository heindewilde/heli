...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  const target = isOutreachTarget(rawTarget) ? rawTarget : null;
  const archivedParam = url.searchParams.get('archived');
  const archived = isArchivedFilter(archivedParam) ? archivedParam : 'active';

  const filtered = archived !== 'active' || !!q || !!platform || !!target;
  ...
  return { items, q, platform, target, archived, total: total ?? items.length };
};
